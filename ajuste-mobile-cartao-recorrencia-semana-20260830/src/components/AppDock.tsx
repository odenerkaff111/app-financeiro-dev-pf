...
export function AppDock() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-[120] hidden border-b border-[#C8A15A]/20 bg-[#071321]/[0.98] shadow-[0_10px_35px_rgba(7,19,33,0.16)] backdrop-blur-2xl md:block">
        <div className="mx-auto flex h-[70px] w-full max-w-[1800px] items-center gap-3 px-5 lg:px-7">
          <Link
            href="/"
      ...
      <header className="fixed inset-x-0 top-0 z-[120] flex h-14 items-center justify-between border-b border-[#C8A15A]/20 bg-[#071321]/[0.98] px-3 shadow-sm backdrop-blur-2xl md:hidden">
        <Link href="/" className="flex items-center gap-2" aria-label="Ir para o painel">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#C8A15A]/35 bg-white/5 text-[#C8A15A]">
            <CircleDollarSign size={17} strokeWidth={2.2} />
          </span>
          <span className="text-xs font-bold tracking-[0.12em] text-white">KYRA</span>
        </Link>
        ...
        <button
          ...
          disabled={loggingOut}
          aria-label="Sair da conta"
          className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 text-white/70 disabled:opacity-50"
        >
          {loggingOut ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : (
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-[125] grid grid-cols-6 border-t border-[#C8A15A]/20 bg-[#071321]/[0.98] px-1 pt-1 shadow-[0_-8px_30px_rgba(7,19,33,0.16)] backdrop-blur-2xl md:hidden"
        ...
        {navigationItems.map((item) => {
          ...
          return (
            <Link
              ...
              aria-current={active ? "page" : undefined}
              className={[
                "flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[9px] font-semibold transition",
                active
                  ? "bg-white/10 text-[#F7F5EF]"
                  : "text-white/58",
              ].join(" ")}
            >
              <Icon
                ...
                className={active ? "text-[#C8A15A]" : ""}
              />
              <span className="max-w-full truncate">{item.compactLabel}</span>
            </Link>
          );
        })}
      </nav>
